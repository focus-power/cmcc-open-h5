...
$(document).ready(function(){
    ...
    var apiDataValue = [
        {"field_datatype_value":"0","field_callnum_value":"123456789012345","field_contractnum_value":"123456789012345"},
    ...
    //产品体验栈事件码
    var product = ['HOME_stack_mbdx','HOME_stack_tyrzbjhmyjdl','HOME_stack_tyrzbjhmjy'];
    // 免费体验按钮
    $('.free-experience').click(function() {
        var indexFree = $(this).attr('data-free');
        var product_code = product[indexFree];
        // alert(product_code)
        Webtrends.multiTrack({argsa: ['DCS.dcsuri', '/nopv.gif', 'WT.event', product_code]});
        if(indexFree == 0) {
            showExperienceModal();  
        } else {
            // window.location.href = './uniform_auth.html'+'?id='+ indexFree;
            showAuthModal();
    ...
    // 点击二维码超时弹框中的“刷新”按钮
    $('.erwei-overtime-refresh').click(function() {
        $(this).parents('.erwei-overtime').removeClass('act');
        $(this).parents('.erwei-overtime').siblings('.erwei-code').addClass('act');
    })
    // 关闭统一认证
    ...
    $(".solution-l li").on("click",function(){
        ...
        var $solution_url = 'solution_new.html?id='+ $solution_api_id +'&WT.ac='+marketing_code_solution;

        //2018.3.15 updata 添加事件码
        var hso_code = data.solution[$solution_api_id].hso_event_code;
        Webtrends.multiTrack({argsa:['DCS.dcsuri','/nopv.gif','WT.event',hso_code]});
         // alert($solution_url);
         // debugger;
    ...
    //关闭首页弹窗
    $('.banner-pop-close').click(function () {
        $('.banner-pop').hide();
    });
...
//如果是手机端 改变定位的right值
    $(function () {
        if(MobileUA.SMART_PHONE){
            $('.banner-pop').css({"right":"-13%"});
        }
    });

    //首页右侧浮窗事件码
    $('.banner-pop-bg').click(function () {
        var pop_code = "MWC_HOME_DJBM";
        // alert(pop_code)
        Webtrends.multiTrack({argsa:['DCS.dcsuri','/nopv.gif','WT.event',pop_code]});
    })
...
function scrollS(){
	...
	/*$('body').on('click', '#sol-l', function(e) {
    //touchstart在你之前发生，不管些什么，都先执行下面的
	});
 
	$('body').on('touchstart', '#sol-l', function(e) {
            var touch = e.originalEvent,
            startX = touch.changedTouches[0].pageX;
            startY = touch.changedTouches[0].pageY;
            $("#sol-l").on('touchend', function(e) {
                e.preventDefault();
                touch = e.originalEvent.touches[0] || e.originalEvent.changedTouches[0];
                if (touch.pageX - startX > 350) {
					if(indexL >  0){
						indexL = indexL-1;
						oneWidth = oneWidth+$li.eq(indexL).outerWidth()*6;
						$(".rCon").stop(true,false).animate({
                ...
                } else if (touch.pageX - startX < -350) {
					if(liSum - scrollSum < screenWidth-280){
						return;
					}else{
						oneWidth = oneWidth - $li.eq(indexL).width()*6;
						$(".rCon").stop(true,false).animate({
